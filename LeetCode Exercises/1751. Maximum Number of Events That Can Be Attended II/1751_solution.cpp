...
public:
    int maxValue(std::vector<std::vector<int>>& events, int k) {
        ...
        std::vector<std::vector<int>> dp(size + 1, std::vector<int>(k + 1));    
        for (int i = 1; i <= size; ++i) {
            // end day of previous event (the other field are not important so fill 0)
            std::vector<int> prev_endday = {0, events[i - 1][0], 0};
            
            // find next event! (the next event that does not overlap with the current one)
            const int next_event = std::lower_bound(begin(events), begin(events) + i,  prev_endday, comp) - begin(events);
            
            // dp[i][j] ==> max value we can get by attending at most j events among events[0..i]
            for (int j = 1; j <= k; ++j) {
                dp[i][j] = std::max(dp[i - 1][j], dp[next_event][j - 1] + events[i - 1][2]);
            }
        }
...
int main() {
    ...
    k = 2;
    std::cout << s.maxValue(input, k) << std::endl;
    return 0;
}